...
func NewLogger(appConf *Config) (*Logger, error) {
	if l, err := initLogrus(appConf); err != nil {
		return nil, errors.New("init logger failed " + err.Error())
	} else {
		return &Logger{l}, nil
	}
}

func initLogrus(appConf *Config) (*logrus.Logger, error) {
	logDir := appConf.GetLogDir()
